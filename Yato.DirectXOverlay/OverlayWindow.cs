    ...
    public class OverlayWindow : IDisposable
    {
        ...
        private void windowThreadMethod(int x = 0, int y = 0, int width = 800, int height = 600)
        {
            ...
            while (true)
            {
                ...
                if (PeekMessageW(ref message, WindowHandle, 0, 0, 1) != 0)
                {
                    if (message.Msg == WindowsMessage.WM_QUIT) return;
        ...
        private void setupInstance(int x = 0, int y = 0, int width = 800, int height = 600)
        {
            ...
            SetLayeredWindowAttributes(WindowHandle, 0, 255, /*0x1 |*/ 0x2);
        ...
        private IntPtr windowProcedure(IntPtr hwnd, WindowsMessage msg, IntPtr wParam, IntPtr lParam)
        {
            switch (msg)
            {
                ...
                case WindowsMessage.WM_ERASEBKGND:
                    SendMessage(WindowHandle, WindowsMessage.WM_PAINT, (IntPtr)0, (IntPtr)0);
                    break;
        ...
        public void extendFrameIntoClientArea()
        {
            ...
            DwmExtendFrameIntoClientArea(WindowHandle, ref margin);
        }
        ...
        {
            MoveWindow(WindowHandle, x, y, Width, Height, 1);
            X = x;
            Y = y;
            extendFrameIntoClientArea();
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                ...
                UnregisterClass(randomClassName, IntPtr.Zero);
        ...
        private static extern void DwmExtendFrameIntoClientArea(IntPtr hWnd, ref MARGIN pMargins);
        ...
        [DllImport("user32.dll", SetLastError = false)]
        private static extern ushort RegisterClassEx(ref WNDCLASSEX wndclassex);

        [DllImport("user32.dll", SetLastError = false, CharSet = CharSet.Unicode)]
        private static extern int UnregisterClass(string lpClassName, IntPtr hInstance);
        ...
        [DllImport("user32.dll", SetLastError = false, CharSet = CharSet.Unicode)]
        private static extern int PeekMessageW(ref Message msg, IntPtr hwnd, uint filterMin, uint filterMax, uint removeMsg);
        ...
        [DllImport("user32.dll", SetLastError = false)]
        private static extern IntPtr DefWindowProc(IntPtr hwnd, WindowsMessage msg, IntPtr wparam, IntPtr lparam);

        [DllImport("user32.dll", SetLastError = false)]
        private static extern int SendMessage(IntPtr hwnd, WindowsMessage msg, IntPtr wparam, IntPtr lparam);
